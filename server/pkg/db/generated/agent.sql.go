...
RETURNING id, agent_id, issue_id, status, priority, dispatched_at, started_at, completed_at, result, error, created_at, context, runtime_id, session_id, work_dir, trigger_comment_id, autopilot_run_id, attempt, max_attempts, parent_task_id, failure_reason, trigger_summary, force_fresh_session, is_leader_task, wait_reason, last_activity_at, role
`
...
const countActiveRetrospectiveRunsByFeature = `-- name: CountActiveRetrospectiveRunsByFeature :one
SELECT count(*)::bigint
...
func (q *Queries) CountActiveRetrospectiveRunsByFeature(ctx context.Context, featureID pgtype.UUID) (int64, error) {
	row := q.db.QueryRow(ctx, countActiveRetrospectiveRunsByFeature, featureID)
	var column_1 int64
	err := row.Scan(&column_1)
	return column_1, err
}

const countActiveValidatorRunsByMilestone = `-- name: CountActiveValidatorRunsByMilestone :one
SELECT count(*)::bigint
FROM agent_task_queue atq
JOIN issue i ON i.id = atq.issue_id
...
func (q *Queries) CountActiveValidatorRunsByMilestone(ctx context.Context, milestoneID pgtype.UUID) (int64, error) {
	row := q.db.QueryRow(ctx, countActiveValidatorRunsByMilestone, milestoneID)
	var column_1 int64
	err := row.Scan(&column_1)
	return column_1, err
}
...
const countRunsByFeature = `-- name: CountRunsByFeature :one
SELECT count(*)::bigint
FROM agent_task_queue atq
JOIN issue i ON i.id = atq.issue_id
WHERE i.feature_id = $1
...
func (q *Queries) CountRunsByFeature(ctx context.Context, featureID pgtype.UUID) (int64, error) {
	row := q.db.QueryRow(ctx, countRunsByFeature, featureID)
	var column_1 int64
	err := row.Scan(&column_1)
	return column_1, err
}
...
const createValidatorTask = `-- name: CreateValidatorTask :one
INSERT INTO agent_task_queue (
    agent_id, runtime_id, issue_id, status, priority, role, force_fresh_session
)
VALUES (
...
type CreateValidatorTaskParams struct {
	AgentID   pgtype.UUID `json:"agent_id"`
	RuntimeID pgtype.UUID `json:"runtime_id"`
	IssueID   pgtype.UUID `json:"issue_id"`
	Priority  int32       `json:"priority"`
...
func (q *Queries) CreateValidatorTask(ctx context.Context, arg CreateValidatorTaskParams) (AgentTaskQueue, error) {
	row := q.db.QueryRow(ctx, createValidatorTask,
		arg.AgentID,
		arg.RuntimeID,
		arg.IssueID,
		arg.Priority,
...
RETURNING t.id, t.agent_id, t.issue_id, t.status, t.priority, t.dispatched_at, t.started_at, t.completed_at, t.result, t.error, t.created_at, t.context, t.runtime_id, t.session_id, t.work_dir, t.trigger_comment_id, t.autopilot_run_id, t.attempt, t.max_attempts, t.parent_task_id, t.failure_reason, t.trigger_summary, t.force_fresh_session, t.is_leader_task, t.wait_reason, t.last_activity_at, t.role
`
...
SELECT id, agent_id, issue_id, status, priority, dispatched_at, started_at, completed_at, result, error, created_at, context, runtime_id, session_id, work_dir, trigger_comment_id, autopilot_run_id, attempt, max_attempts, parent_task_id, failure_reason, trigger_summary, force_fresh_session, is_leader_task, wait_reason, last_activity_at, role FROM agent_task_queue
WHERE id = $1
`
...
const getFeatureTripwireUsage = `-- name: GetFeatureTripwireUsage :one
SELECT
    COALESCE((
...
type GetFeatureTripwireUsageRow struct {
...
func (q *Queries) GetFeatureTripwireUsage(ctx context.Context, featureID pgtype.UUID) (GetFeatureTripwireUsageRow, error) {
	row := q.db.QueryRow(ctx, getFeatureTripwireUsage, featureID)
	var i GetFeatureTripwireUsageRow
	err := row.Scan(&i.TokensUsed, &i.ElapsedSeconds)
	return i, err
}
...
const getValidatorAgent = `-- name: GetValidatorAgent :one
...
type GetValidatorAgentParams struct {
	WorkspaceID pgtype.UUID `json:"workspace_id"`
	ID          pgtype.UUID `json:"id"`
}
...
func (q *Queries) GetValidatorAgent(ctx context.Context, arg GetValidatorAgentParams) (pgtype.UUID, error) {
	row := q.db.QueryRow(ctx, getValidatorAgent, arg.WorkspaceID, arg.ID)
	var id pgtype.UUID
	err := row.Scan(&id)
	return id, err
}
...
SELECT atq.id, atq.agent_id, atq.issue_id, atq.status, atq.priority, atq.dispatched_at, atq.started_at, atq.completed_at, atq.result, atq.error, atq.created_at, atq.context, atq.runtime_id, atq.session_id, atq.work_dir, atq.trigger_comment_id, atq.autopilot_run_id, atq.attempt, atq.max_attempts, atq.parent_task_id, atq.failure_reason, atq.trigger_summary, atq.force_fresh_session, atq.is_leader_task, atq.wait_reason, atq.last_activity_at, atq.role FROM agent_task_queue atq
JOIN agent a ON a.id = atq.agent_id
WHERE a.workspace_id = $1
...
SELECT t.id, t.agent_id, t.issue_id, t.status, t.priority, t.dispatched_at, t.started_at, t.completed_at, t.result, t.error, t.created_at, t.context, t.runtime_id, t.session_id, t.work_dir, t.trigger_comment_id, t.autopilot_run_id, t.attempt, t.max_attempts, t.parent_task_id, t.failure_reason, t.trigger_summary, t.force_fresh_session, t.is_leader_task, t.wait_reason, t.last_activity_at, t.role FROM (
  SELECT DISTINCT ON (atq.agent_id) atq.id, atq.agent_id, atq.issue_id, atq.status, atq.priority, atq.dispatched_at, atq.started_at, atq.completed_at, atq.result, atq.error, atq.created_at, atq.context, atq.runtime_id, atq.session_id, atq.work_dir, atq.trigger_comment_id, atq.autopilot_run_id, atq.attempt, atq.max_attempts, atq.parent_task_id, atq.failure_reason, atq.trigger_summary, atq.force_fresh_session, atq.is_leader_task, atq.wait_reason, atq.last_activity_at, atq.role
  FROM agent_task_queue atq
  JOIN agent a ON a.id = atq.agent_id
  WHERE a.workspace_id = $1
...
const stampTaskActivity = `-- name: StampTaskActivity :exec
UPDATE agent_task_queue
...
func (q *Queries) StampTaskActivity(ctx context.Context, id pgtype.UUID) error {
	_, err := q.db.Exec(ctx, stampTaskActivity, id)
	return err
}
...
const waitTaskForLocalDirectory = `-- name: WaitTaskForLocalDirectory :one
UPDATE agent_task_queue
...
type WaitTaskForLocalDirectoryParams struct {
	ID         pgtype.UUID `json:"id"`
...
func (q *Queries) WaitTaskForLocalDirectory(ctx context.Context, arg WaitTaskForLocalDirectoryParams) (AgentTaskQueue, error) {
	row := q.db.QueryRow(ctx, waitTaskForLocalDirectory, arg.ID, arg.WaitReason)
	var i AgentTaskQueue
	err := row.Scan(
		&i.ID,
		&i.AgentID,

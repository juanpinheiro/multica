...
INSERT INTO autopilot (
    ...
    $5, $6, $10, $11,
    $7, $8
) RETURNING id, workspace_id, title, description, assignee_id, status, execution_mode, issue_title_template, created_by_type, created_by_id, last_run_at, created_at, updated_at, assignee_type, feature_id
`
...
SELECT id, workspace_id, title, description, assignee_id, status, execution_mode, issue_title_template, created_by_type, created_by_id, last_run_at, created_at, updated_at, assignee_type, feature_id FROM autopilot
WHERE id = $1
`
...
const setAutopilotTriggerEventFilters = `-- name: SetAutopilotTriggerEventFilters :one
UPDATE autopilot_trigger
SET event_filters = $2,
    updated_at = now()
WHERE id = $1
...
type SetAutopilotTriggerEventFiltersParams struct {
	ID           pgtype.UUID `json:"id"`
	EventFilters []string    `json:"event_filters"`
}
...
func (q *Queries) SetAutopilotTriggerEventFilters(ctx context.Context, arg SetAutopilotTriggerEventFiltersParams) (AutopilotTrigger, error) {
	row := q.db.QueryRow(ctx, setAutopilotTriggerEventFilters, arg.ID, arg.EventFilters)
	var i AutopilotTrigger
	err := row.Scan(
		&i.ID,
		&i.AutopilotID,

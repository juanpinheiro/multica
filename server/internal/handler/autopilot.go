...
func autopilotToResponse(a db.Autopilot) AutopilotResponse {
	...
	return AutopilotResponse{
		...
		FeatureID:          uuidToPtr(a.FeatureID),
...
func (h *Handler) CreateAutopilot(w http.ResponseWriter, r *http.Request) {
	...
	projectID, ok := h.parseAutopilotFeatureID(w, r, req.FeatureID, wsUUID)
	if !ok {
		return
	}
...
func (h *Handler) UpdateAutopilot(w http.ResponseWriter, r *http.Request) {
	...
	if _, ok := rawFields["feature_id"]; ok {
		projectID, ok := h.parseAutopilotFeatureID(w, r, req.FeatureID, prev.WorkspaceID)
		if !ok {
			return
		}
		params.FeatureID = projectID
	}
	// assignee_type and assignee_id are validated as a pair. The client is
	// expected to send both fields on any change; partial updates that change
	// only one are
	// rejected.
	...
	if typeSent || idSent {
		...
		// Reject assignee_type change without a paired id.
...
func (h *Handler) parseAutopilotFeatureID(
	...
	projectID, ok := parseUUIDOrBadRequest(w, *raw, "feature_id")
	if !ok {
	...
	if _, err := h.Queries.GetFeatureInWorkspace(r.Context(), db.GetFeatureInWorkspaceParams{
		...
		writeError(w, http.StatusBadRequest, "feature_id must reference a project in this workspace")
		return pgtype.UUID{}, false
	}
...
// validateAutopilotAssignee checks that the agent assignee exists in the
// given workspace and is in a workable state. Writes an HTTP error and

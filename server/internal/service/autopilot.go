...
// The same archived / runtime-bound / runtime-online gates that the upstream admission check (shouldSkipDispatch)
...
// task — i.e. the assignee is gone, archived, has no runtime bound, or its
// runtime is not currently online. Returns ("", false) on the happy path.
//
...
func (s *AutopilotService) shouldSkipDispatch(ctx context.Context, ap db.Autopilot) (string, bool) {
	// Initiative gate: dispatch is blocked unless the parent Initiative
	...
	if ap.FeatureID.Valid {
		...
		if initiative.Status(feature.Status) != initiative.StatusRunning {
			return "feature_not_running", true
		}
	}
	...
	if err != nil {
		...
		if missing {
			// Agent row gone — skip rather than fail-open: retrying will not help.
			return "assignee agent no longer exists", true
		}
		// Transient DB error — fail-open so the next scheduler tick gets a chance to succeed.
		return "", false
	}
	ready, reason, err := AgentReadiness(ctx, s.Queries, agent)
	if err != nil {
...
func formatAdmissionReason(ap db.Autopilot, raw string) string {
	...
	case "agent is archived":
		return "assignee agent is archived"
	case "agent has no runtime bound":
		return "assignee agent has no runtime bound"
	default:
...
// resolveAutopilotLeader returns the agent that will execute the autopilot's work.
func (s *AutopilotService) resolveAutopilotLeader(ctx context.Context, ap db.Autopilot) (agent db.Agent, agentResolved bool, err error) {

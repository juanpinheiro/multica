...
import {
  featureResourcesOptions,
  useCreateFeatureResource,
...
// Feature Resources sidebar section.
//
...
export function FeatureResourcesSection({ featureId }: { featureId: string }) {
  ...
  const { data: resources = [] } = useQuery(
    featureResourcesOptions(wsId, featureId),
  );
  const createResource = useCreateFeatureResource(wsId, featureId);
  const deleteResource = useDeleteFeatureResource(wsId, featureId);
  ...
  const handleRemove = async (resource: FeatureResource) => {
    try {

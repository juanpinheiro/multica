"use client";

import { useQuery } from "@tanstack/react-query";
import { featureListOptions, featureDetailOptions } from "@multica/core/features/queries";
import { useWorkspaceId } from "@multica/core/hooks";
import { FeatureIcon } from "./feature-icon";
import { useT } from "../../i18n";

/**
 * Compact presentational representation of a feature —
 * `<emoji> <title>`, bordered, truncating to max-w-72. Mirror of IssueChip.
 *
 * Not a link / button: callers wrap it in whatever interactive shell they
 * need. Pure UI — data is queried internally so callers can pass just an id.
 */
export interface FeatureChipProps {
  featureId: string;
  /** Shown when the feature can't be resolved. */
  fallbackLabel?: string;
  /** Extra classes — callers layer interaction hints here. */
  className?: string;
}

const BASE_CLASS =
  "feature-chip inline-flex items-center gap-1.5 rounded-md border mx-0.5 px-2 py-0.5 text-xs max-w-72";

export function FeatureChip({
  featureId,
  fallbackLabel,
  className,
}: FeatureChipProps) {
  const { t } = useT("features");
  const wsId = useWorkspaceId();
  const { data: features = [] } = useQuery(featureListOptions(wsId));
  const listFeature = features.find((p) => p.id === featureId);

  const { data: detailFeature } = useQuery({
    ...featureDetailOptions(wsId, featureId),
    enabled: !listFeature,
  });

  const feature = listFeature ?? detailFeature;
  const cls = className ? `${BASE_CLASS} ${className}` : BASE_CLASS;

  if (!feature) {
    return (
      <span className={cls}>
        <FeatureIcon size="md" />
        <span className="text-muted-foreground truncate">
          {fallbackLabel ?? t(($) => $.chip.fallback_label)}
        </span>
      </span>
    );
  }

  return (
    <span className={cls}>
      <FeatureIcon feature={feature} size="md" />
      <span className="text-foreground truncate">{feature.title}</span>
    </span>
  );
}

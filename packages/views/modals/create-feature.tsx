...
import { useCreateFeature } from "@multica/core/features/mutations";
import { useFeatureDraftStore } from "@multica/core/features";
import {
...
} from "../features/components/labels";
...
export function CreateFeatureModal({ onClose }: { onClose: () => void }) {
  const { t } = useT("modals");
  ...
  const projectStatusLabels = useFeatureStatusLabels();
  const projectPriorityLabels = useFeaturePriorityLabels();

  const draft = useFeatureDraftStore((s) => s.draft);
  const setDraft = useFeatureDraftStore((s) => s.setDraft);
  const clearDraft = useFeatureDraftStore((s) => s.clearDraft);

  const [title, setTitle] = useState(draft.title);
  const descEditorRef = useRef<ContentEditorRef>(null);
  const [status, setStatus] = useState<FeatureStatus>(draft.status);
  const [priority, setPriority] = useState<FeaturePriority>(draft.priority);
  ...
  // Repos selected to attach as github_repo resources after the feature is
  // created. Stored as URLs (not full FeatureResource rows) — they're not
  // persisted until handleSubmit fires the createFeatureResource calls.
  ...
  const updateStatus = (v: FeatureStatus) => { setStatus(v); setDraft({ status: v }); };
  const updatePriority = (v: FeaturePriority) => { setPriority(v); setDraft({ priority: v }); };
  ...
  const leadLabel =
    leadType && leadId ? getActorName(leadType, leadId) : t(($) => $.create_feature.lead);
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      const newFeature = await createFeature.mutateAsync({
        title: title.trim(),
        ...
        lead_type: leadType,
        lead_id: leadId,
        // Server attaches these in the same transaction as the feature.
        resources:
      ...
      clearDraft();
      onClose();
      toast.success(t(($) => $.create_feature.toast_created));
      router.push(wsPaths.featureDetail(newFeature.id));
    } catch (err) {
      toast.error(
        err instanceof Error && err.message
          ? err.message
          : t(($) => $.create_feature.toast_failed),
      );
    } finally {
      setSubmitting(false);
    }
  ...
  return (
    <Dialog open onOpenChange={(v) => { if (!v) onClose(); }}>
      ...
      >
        <DialogTitle className="sr-only">{t(($) => $.create_feature.title)}</DialogTitle>

        <div className="flex items-center justify-between px-5 pt-3 pb-2 shrink-0">
          <div className="flex items-center gap-1.5 text-xs">
            <span className="text-muted-foreground">{workspaceName}</span>
            <ChevronRight className="size-3 text-muted-foreground/50" />
            <span className="font-medium">{t(($) => $.create_feature.title_breadcrumb)}</span>
          </div>
        ...
        <div className="px-5 pb-2 shrink-0">
            ...
            <PopoverTrigger
              render={
                <button
                  ...
                  title={t(($) => $.create_feature.icon_tooltip)}
                >
                  {icon || "📁"}
                </button>
              }
          ...
          <TitleEditor
            ...
            placeholder={t(($) => $.create_feature.title_placeholder)}
        ...
        <div className="flex-1 min-h-0 overflow-y-auto px-5">
          <ContentEditor
            ...
            placeholder={t(($) => $.create_feature.description_placeholder)}
        ...
        <div className="flex items-center justify-between gap-2 px-4 py-3 border-t shrink-0">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger
              render={
                <PillButton>
                  <span className={cn("size-2 rounded-full", FEATURE_STATUS_CONFIG[status].dotColor)} />
              ...
              render={
                <PillButton>
                  {leadType && leadId ? (
                    ...
                    <span className="text-muted-foreground">{t(($) => $.create_feature.lead)}</span>
                  )}
                </PillButton>
              }
            />
            <PopoverContent align="start" className="w-52 p-0">
              <div className="px-2 py-1.5 border-b">
                <input
                  ...
                  placeholder={t(($) => $.create_feature.lead_placeholder)}
              ...
              <div className="p-1 max-h-60 overflow-y-auto">
                ...
                >
                  ...
                  <span className="text-muted-foreground">{t(($) => $.create_feature.no_lead)}</span>
                </button>
                {filteredMembers.length > 0 && (
                  <>
                    <div className="px-2 pt-2 pb-1 text-xs font-medium text-muted-foreground uppercase tracking-wider">
                      {t(($) => $.create_feature.members_group)}
                    </div>
          ...
          >
            <PopoverTrigger
              render={
                <PillButton>
                  ...
                  <span>
                    {selectedRepos.length === 0
                      ? t(($) => $.create_feature.repos_pill)
                      : t(($) => $.create_feature.repos_pill_count, { count: selectedRepos.length })}
                  </span>
                </PillButton>
              }
            />
            <PopoverContent align="start" className="w-72 p-2 space-y-2">
              <div className="text-xs font-medium text-muted-foreground">
                {t(($) => $.create_feature.repos_heading)}
              </div>
                ...
                <>
                  <div className="relative">
                    ...
                    <input
                      ...
                      aria-label={t(($) => $.create_feature.repos_search_placeholder)}
                      placeholder={t(($) => $.create_feature.repos_search_placeholder)}
                  ...
                  <div className="max-h-48 space-y-1 overflow-y-auto">
                    {filteredWorkspaceRepos.length === 0 && repoQuery && (
                      <p className="py-2 text-center text-xs text-muted-foreground">
                        {t(($) => $.create_feature.repos_search_empty)}
                      </p>
                    )}
              ...
              >
                <input
                  ...
                  placeholder={t(($) => $.create_feature.repos_url_placeholder)}
                ...
                >
                  {t(($) => $.create_feature.repos_add)}
                </Button>
              </form>
              {selectedRepos.length > 0 && (
                <div className="space-y-1 pt-1 border-t">
                  <div className="text-[10px] font-medium text-muted-foreground uppercase tracking-wider">
                    {t(($) => $.create_feature.repos_selected)}
                  </div>
                  {selectedRepos.map((url) => (
                    <div
                      key={url}
          ...
          <Button
            ...
            className="shrink-0"
          >
            {submitting ? t(($) => $.create_feature.submitting) : t(($) => $.create_feature.submit)}
          </Button>
        </div>
      </DialogContent>
    </Dialog>

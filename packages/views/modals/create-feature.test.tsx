...
vi.mock("@multica/core/features", () => ({
  useFeatureDraftStore: (selector: (state: unknown) => unknown) =>
    selector({
      draft: {
        title: "",
        description: "",
...
describe("CreateFeatureModal", () => {
  it("exposes full repository URLs in the repository picker", () => {
    render(<CreateFeatureModal onClose={vi.fn()} />);
  ...
  it("filters workspace repositories by search text", async () => {
    ...
    renderWithI18n(<CreateFeatureModal onClose={vi.fn()} />);

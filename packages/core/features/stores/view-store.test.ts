...
import { useFeatureViewStore } from "./view-store";
...
beforeEach(() => {
  localStorage.clear();
  useFeatureViewStore.setState({ viewMode: "compact" });
...
describe("useFeatureViewStore", () => {
  it("defaults to 'compact'", () => {
    expect(useFeatureViewStore.getState().viewMode).toBe("compact");
  });

  it("setViewMode mutates the store", () => {
    useFeatureViewStore.getState().setViewMode("comfortable");
    expect(useFeatureViewStore.getState().viewMode).toBe("comfortable");
  });
